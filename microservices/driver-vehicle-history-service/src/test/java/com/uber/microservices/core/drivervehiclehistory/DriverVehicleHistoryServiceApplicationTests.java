...
public class DriverVehicleHistoryServiceApplicationTests {
	...
	private WebTestClient.BodyContentSpec postAndVerifyDriverVehicleHistory(int driverId, int vehicleId, HttpStatus expectedStatus) {
		DriverVehicleHistory history = new DriverVehicleHistory(driverId, vehicleId, LocalDate.now(), Constants.END_OF_TIME);
		return client.post()
				.uri("/driver-vehicle-history")
				.body(just(history), DriverVehicleHistory.class)
				.accept(APPLICATION_JSON)
				.exchange()
				.expectStatus().isEqualTo(expectedStatus)
				.expectHeader().contentType(APPLICATION_JSON)
